...
const char * const BITCOIN_CONF_FILENAME = "exucoin.conf";
const char * const BITCOIN_PID_FILENAME = "exucoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "exucoin" is a composite category enabling all EXUCOIN-related debug output
                if(ptrCategory->count(std::string("exucoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EXUCOIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EXUCOIN
    // Mac: ~/Library/Application Support/EXUCOIN
    // Unix: ~/.exucoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EXUCOIN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EXUCOIN";
#else
    // Unix
    return pathRet / ".exucoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty exucoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override exucoin.conf
